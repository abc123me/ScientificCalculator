...
public class StatisticsPanel extends JComponent {
	...
	public UnaryFunction asYEqualsAXPlusB(int xList, int yList){
		Vector[] points = getListsAsPointsArray(xList, yList);
		float avgSlope = 0, avgIntercept = 0;
		Vector lastPoint = null, currentPoint = null;
		for(int i = 1; i < points.length; i++){
			lastPoint = points[i - 1];
			currentPoint = points[i];
			avgSlope += VectorMath.slope(lastPoint, currentPoint);
			avgIntercept += VectorMath.yIntercept(lastPoint, currentPoint);
		}
		avgSlope /= points.length;
		avgIntercept /= points.length;
		return FunctionFactory.line(avgSlope, avgIntercept);
	}
	public Vector[] getListsAsPointsArray(int xList, int yList){
		float[] xValues = new float[lists[xList].base.size()];
		float[] yValues = new float[lists[yList].base.size()];
		int xLen = xValues.length, yLen = yValues.length;
		for(int i = 0; i < xLen; i++)
			xValues[i] = lists[xList].base.get(i).floatValue();
		for(int i = 0; i < yLen; i++)
			yValues[i] = lists[yList].base.get(i).floatValue();
		if(xLen != yLen){
			int minLen = xLen < yLen ? xLen : yLen;
			int maxLen = xLen > yLen ? xLen : yLen;
			float[] resized = new float[maxLen];
			for(int i = 0; i < minLen; i++)
				resized[i] = (xLen < yLen ? xValues : yValues)[i];
			for(int i = minLen - 1; i < maxLen; i++)
				resized[i] = 0;
		}
		return VectorMath.combineXYArrays(xValues, yValues);
	}
}
